(function () {
    ...
    app.controller('schedulesController', ['$scope', '$http', '$window', function ($scope, $http, $window) {
        ...
        $scope.getSchedule = function (schedulId) {
            if (schedulId) {
        ...
        $scope.isStudentSelected = isStudentSelected;
            ...
            $http.get('/api/SchedulesAPI/GetWeekDays')
                .then(function (response) {
                    ...
                    if (!$scope.schedule.WeekDay) {
                        $scope.schedule.WeekDay = $scope.weekDays[0];
                    }
                });
        }

        function getCourses() {
            $http.get('/api/CoursesAPI/Get')
                .then(function (response) {
        ...
        function isStudentSelected(student) {
            return $scope.schedule.Students.indexOf(student) > -1;
        }
